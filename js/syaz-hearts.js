...
export function mountSyazHeartGrid(container) {
  ...
  function tick() {
    ...
    hearts.forEach(el => {
      ...
      const base = 0.32 + pulse * 0.68;

      el.style.opacity = base;
      el.style.color = pulse > 0.3 ? '#fff' : 'rgba(210, 170, 255, 0.95)';
      el.style.transform = `scale(${0.7 + pulse * 0.55})`;
      el.style.filter = pulse > 0.25
        ? `drop-shadow(0 0 ${4 + pulse * 8}px rgba(255,140,220,${0.45 + pulse * 0.55}))`
        : 'drop-shadow(0 0 2px rgba(255,107,203,0.2))';
    });
